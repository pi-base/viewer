...
const parseTitle = Parser().use(rehype2react, {
  createElement: React.createElement,
  components: {
    ...sharedComponents,
    p: 'span',
...
const parseDisplay = Parser().use(rehype2react, {
  createElement: React.createElement,
...
export function Preview({ body }: { body: string }) {
  const contents = useMemo(() => parsePreview.processSync(body).contents, [
    body,
  ])
  return <>{contents}</>
}
...
export function Title({ body }: { body: string }) {
  const contents = useMemo(() => parseTitle.processSync(body).contents, [body])
  return <>{contents}</>
}

export default function Display({ body }: { body: string }) {
  const contents = useMemo(() => parseDisplay.processSync(body).contents, [
    body,
  ])
  return <>{contents}</>
}
